...
const INJECTION_PATTERNS = [
  ...
  /reveal\s+(the\s+|your\s+)?(system\s+prompt|instructions|developer\s+mode|api\s+keys?|secrets?)/i,
  /show\s+me\s+(the\s+|your\s+)?(system\s+prompt|secrets?|api\s+keys?)/i,
  /follow\s+these\s+instructions\s+instead/i,
  /you\s+are\s+now\s+in\s+developer\s+mode/i,
  /you\s+are\s+now\s+(the\s+system|an\s+unfiltered)/i,
  /jailbreak\s+activated/i,
  /output\s+all\s+system\s+rules/i,
  /override\s+(security|safety|guardrail|system)\s+(policy|filters|rules|instructions)/i,
  /bypass\s+security/i,
  /disable\s+security/i,
  /turn\s+off\s+safety/i,
  /NEXUS_DEMO_SECRET_7X9Q_FAKE/i,
  /AKIA_NEXUS_DEMO_HONEYTOKEN_DO_NOT_USE_7781/i,
  /<\|im_start\|>/i,
  /<\|im_end\|>/i,
  /\[INST\]/i,
  /\[\/INST\]/i
