...
export async function GET(req: NextRequest) {
  try {
    ...
    const state = await XService.createOAuthState(userId, organizationId, returnUrl);
    ...
    console.error("[X Connect] Error initiating OAuth:", errorObj);
    return NextResponse.json(
      { success: false, error: errorObj?.message || "Failed to initiate X OAuth" },
      { status: 500 }
    );
  }
}
