...
export async function POST(
  ...
  try {
    ...
    const reason =
      ...
      "Rejected via NEXUS Governance Console";
    const reviewerId =
    ...
    return NextResponse.json({
      ...
      approvalId,
      message: "Content rejected and returned to author.",
    });
  } catch (err: unknown) {
    const errObj = err as Error;
    return NextResponse.json(
      { success: false, error: errObj?.message || "Error submitting rejection decision" },
      { status: 500 }
    );
  }
}
